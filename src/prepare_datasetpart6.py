...
sample_ids = np.array([p.name for p in sample_dirs])
cluster_labels = np.array(cluster_labels)

logo = LeaveOneGroupOut()
folds_data = {}

print("\nGerando partições Leave-One-Cluster-Out (n-1 treino, 1 teste):")

for fold_idx, (train_val_idx, test_idx) in enumerate(logo.split(sample_ids, groups=cluster_labels)):
    held_out_cluster = int(cluster_labels[test_idx[0]])
    
    train_val_ids = sample_ids[train_val_idx]
    train_val_groups = cluster_labels[train_val_idx]
    test_ids = sample_ids[test_idx].tolist()
    
    # Separa 15% do conjunto de treino restante para validação (estratificado entre os clusters que sobraram)
    ...
    folds_data[f"fold_test_cluster_{held_out_cluster}"] = {
        "test_cluster": held_out_cluster,
        "train": train_ids.tolist(),
        "val": val_ids.tolist(),
        "test": test_ids
    }
    
    print(f" - Fold {fold_idx + 1} (Testando no Cluster {held_out_cluster}): "
          f"Treino={len(train_ids)}, Val={len(val_ids)}, Teste={len(test_ids)}")

# Salva todos os folds gerados em um único arquivo
split_logo_path = OUTPUT_DIR / "split_leave_one_cluster_out.json"
with open(split_logo_path, "w") as f:
    json.dump(folds_data, f, indent=4)

print(f"\nMapeamento dos folds de validação cruzada salvo com sucesso em: {split_logo_path}")